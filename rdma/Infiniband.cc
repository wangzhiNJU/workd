...
Infiniband::create_queue_pair(ibv_qp_type type)
{
  ...
  Worker* worker = (static_cast<RDMAStack*>(stack))->get_worker();
  Infiniband::QueuePair *qp = new QueuePair(*this, type, ib_physical_port, srq, (static_cast<RDMAWorker*>(worker))->get_tx_cq(), cq, max_send_wr, max_recv_wr);
  if (qp->init()) {
    delete qp;
    return NULL;
  }
...
Infiniband::CompletionQueue* Infiniband::create_comp_queue(CompletionChannel *cc)
{
  ...
  Infiniband::CompletionQueue *cq = new Infiniband::CompletionQueue(*this, max_cqe, cc);
  if (cq->init()) {
    delete cq;
    return NULL;
  }
