...
{
  RDMAConnectedSocketImpl* p = new RDMAConnectedSocketImpl(cct, infiniband);
  entity_addr_t sa;
...
void RDMAWorker::initialize() { 
  ...
  ldout(cct, 20) << __func__ << " going to create file event." << dendl;
  center.create_file_event(tx_cc->get_fd(), EVENT_READABLE, tx_handler);
  ldout(cct, 20) << __func__ << " finished creating file event. " << dendl;
}

void RDMAWorker::handle_tx_event() {
  ldout(cct, 20) << __func__ << dendl;
  while (!tx_cc->get_cq_event())
    return ;
...
again:
  ...
  for(int i=0;i<n;++i){
    ...
    lderr(cct) << __func__ << " opcode: " << response->opcode << " len: " << response->byte_len << dendl;
    if (response->status != IBV_WC_SUCCESS){
      lderr(cct) << __func__ << " poll cqe failed! " << " number: " << n << ", status: "<< response->status << cpp_strerror(errno) << dendl;
      assert(0);
    }
    infiniband->get_memory_manager()->return_channel(chunk); 
  }
